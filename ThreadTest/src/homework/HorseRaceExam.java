...
public class HorseRaceExam {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < horseList.size(); i++) {
			horseList.get(i).start();   //  0~리스트의 마지막까지 실행됨  0~9까지 1번말에서 10번말까지 차례대로 실행됨
		}
	...
	public void run() {
		...
		while (true) {
			...
			for (int i = 0; i < HorseRaceExam.horseList.size(); i++) {
				...
				if (horse.getHorsePosition() != 49) { // 아직 도착하지 않은 경우...
					System.out.print(horse.getHorseName() + " : ");
					System.out.print(horseCourse.substring(0, horse.getHorsePosition()) + ">");
					System.out.println(horseCourse.substring(horse.getHorsePosition()+1, 50));

				} else { // 도착한 경우...
					System.out.print(horse.getHorseName() + " : ");
					System.out.print(horseCourse.substring(0, horse.getHorsePosition()+1) + " [도착]");
					System.out.println();
