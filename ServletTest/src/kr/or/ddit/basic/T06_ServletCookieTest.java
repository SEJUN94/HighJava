...
/*
	...
	웹서버와 브라우저는 애플리케이션을 사용하는 동안 필요한 값을  쿠키를 통해 공유여 상태를 유지한다.
 ...
 */
		...
		if(cookies != null) {
			...
			for(int i=0; i<cookies.length; i++) {
				...
				if((cookie.getName()).equals("userId")) {
					...
					out.println("삭제한 쿠키 : " + cookie.getName() 
								+ "<br>");
				}
	...
	/*
	   ...
	   쿠키값은 사용불가문자를 제외한 나머지 출력가능한 아스키 문자 사용 가능함.
	    => 이외의 값(예를들면 한글)을 사용시에는 URLEcoder.encode() 사용하여 인코딩 처리해 준다. 
	   2. 쿠키 최대 지속시간을 설정한다.(초단위) => 지정하지 않으면 웹 브라우저를 종료할 때 쿠키를
	     함께 삭제한다.
	 ...
	 => 출력 버퍼가 플러시 된 이후에는 쿠키를 추가할 수 없다.
	 (응답헤더를 통해서 브라우저레 전달하기때문에...)
	*/
		// 쿠키생성하기
		...
		// 웹브라우저 콘솔창에서 접근을 막음 name은 콘솔창에서 "document.cookie"를 입력해서 접근 가능
