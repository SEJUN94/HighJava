...
public class UploadServlet extends HttpServlet{
	...
	private static final int MEMORY_THRESHOLD = 1024*1024*3;   // 파일 크기제한
	// 파일 1개당 최대 크기
	private static final long MAX_FILE_SIZE = 1024*1024*40;  // 파일 사이즈 제한
	// 요청 파일 최대 크기
	private static final long MAX_REQUEST_SIZE = 1024*1024*50;  // 한개에 SIZE에서 정한거처럼 40mb까지 허용하지만 한번 REQUEST할때 허용하는건 50mb까지 허용한다
			...
			try {
				...
				if(formItems != null && formItems.size() >0) {
					for (FileItem item : formItems) {
						if(!item.isFormField()) { // 폼필드가 아니면 파일 / 파일인 경우...
