...
public class ParsingMovie {
	public void parse() {
		try {
			...
			String recipeId = "73699";	// 래시피가 궁금한 음식 ID
			...
			if(code.equals("INFO-000")) { // 정상이면...
				for(int i=0; i<rowNodeList.getLength(); i++) {
					...
					String irdntCode = rowEl.getElementsByTagName("IRDNT_TY_CODE").item(0).getTextContent();
					String irdntTyNm= rowEl.getElementsByTagName("IRDNT_TY_NM").item(0).getTextContent();
					String formatStr = String.format("%3s %8s %10s %10s %15s", rowNum, recipeId, irdntTyNm, irdntNm, irdntCpcty);
