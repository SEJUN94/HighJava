...
/*
 * 드라이버를 불러와서 잘불러와지면 잘설치된것
 * 불러와지지 않으면 예외처리를 해준다.
 * getConnection을 통해 얻어옴
 * statement객체를 생성 쿼리를 날릴 객체
 * 관련된 쿼리 statement에 작성하고
 * 다쓴 객체는 close()를 써서 쓸대없는 메모리가 흘러가지않게  닫아줌
 */
public class MemberMain {
	...
	/*
		검색할 회원ID, 회원이름, 전화번호, 주소 등을 입력하면 입력한 정보만
		사용하여 검색하는 기능을 구현하시오.
	 ...
	 */
		...
		if(memList.size() == 0) {
			System.out.println("검색된 회원의 정보가 없습니다.");
		}else {
	...
	private void deleteMember() {
		...
		if(cnt > 0) {
			System.out.println(memId + " 회원 정보 삭제했습니다.");
		}else {
...
boolean chk = false;
		...
		if(cnt > 0) {
			System.out.println(memId + " 회원 정보를 수정 했습니다.");
		}else {
			System.out.println(memId + " 회원 정보 수정을 실패했습니다!!!");
		}
	}
